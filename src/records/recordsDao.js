const recordsDao = {
    selectRecordsByUserId: async (connection, userId, isMine) => {
        const sql = `SELECT cr.record_id, cr.author, cr.created_at, cr.status, cr.date, cr.place_id, places.name, places.category, cr.isbn, cr.activities, cr.emotions, cr.contents, cr.comment_not_allowed, cr.comment_count, cr.like_count, cr.images_url
    ...
    selectRecordsAll: async (connection, userId) => {
        const sql = `SELECT cr.record_id, friends.user_id, cr.created_at, cr.status, cr.date, cr.place_id, places.name, places.category, cr.isbn, cr.activities, cr.emotions, cr.contents, cr.comment_not_allowed, cr.comment_count, cr.like_count, cr.images_url
        ...
        ON cr.place_id = places.place_id AND cr.author = friends.user_id
        WHERE cr.isNotPublic = 0
        ORDER BY cr.created_at DESC`;
        try {
            const [records] = await connection.query(sql);
