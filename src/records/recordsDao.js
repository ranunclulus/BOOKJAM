...
const recordsDao = {
  ...
  insertRecord: async (connection, recordData) => {
    ...
    try {
      ...
      if (!check) await connection.query(sql_bookToPlace, [recordData[1], recordData[2]]);
  ...
  selectRecordsAll: async (connection, userId, last) => {
    const sql = `SELECT cr.record_id, friends.user_id, friends.username, friends.profile_image, cr.created_at, cr.status, cr.date, cr.place_id, places.name, places.category, cr.isbn, cr.activities, cr.emotions, cr.contents, cr.comment_not_allowed, cr.comment_count, cr.like_count, cr.images_url, COUNT(likes.record_id) AS liked
        ...
        JOIN (SELECT place_id, category, name FROM places) AS places 
        JOIN (SELECT user_id, username, profile_image FROM follow f JOIN users u ON f.followee = u.user_id WHERE follower = ${userId} UNION SELECT user_id, username, profile_image FROM users WHERE user_id = ${userId}) AS friends
        LEFT JOIN (SELECT record_id FROM record_likes WHERE liker = ${userId}) AS likes
